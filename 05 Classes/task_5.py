...
class Ads(Writer):
    def __init__(self):
        ...
        while True:
            ...
            try:
                datetime.strptime(self.exp_date, '%Y-%m-%d')
                break
            except ValueError:
                print("Incorrect date, please enter a date in the format 'yyyy-mm-dd'")
...
class Input:
    ...
    def process_input(self):
        ...
        if self.type == 1:
            ...
            # raise allert if input integer not between 1-4
...
if __name__ == '__main__':
    ...
    # This object's process_input method is then called as long as the feed type is not 4 (to exit)
    # If the user's input is invalid, an error message is printed and the user is prompted again
    while True:
        try:
            start_app = int(input('Enter type of input: 1) news, 2) ads, 3) forecast, 4) exit: '))
            if start_app == 4:
                print('Application closed')
                break
            else:
